...
{    
    const bg_video_container = useRef(null)
    useEffect( () => { get_bg_video(bg_video_container.current.getAttribute("data-video-id"), 'json') }, [])
    
    async function get_bg_video(video_id, format) {
        try {
            const response = await axios.get(`https://vimeo.com/api/oembed.${format}?url=https://vimeo.com/${video_id}&background=true`)
            window.requestAnimationFrame( () => {
                bg_video_container.current.insertAdjacentHTML("beforeend", response.data.html)
            })
        }
    ...
    content={
        <div id='start_page_content'>
            <span>
                <h2>Центр иностранных<br/>языков </h2><h1 id="inspiration_title"/>
            </span>
            <img src={quotes} className='quote_icon'/>
            <p id='main_page_quote'>Ошибки - это всего лишь часть <u>обучения.</u>
            </p>
            <button id='main_page_action' onClick={ () => {
                scrollToAnchor('courses_page')
            }}>
                <strong>Записаться на занятие</strong>
                <div>
            ...
            <svg id='main_page_arrow_down' viewBox="0 0 56 31">
    ...
    index = {0}
    background_content={
    <div id='video_background' ref={bg_video_container} data-video-id = '943528447'>
        <img id='frame_video_bg' src={poster}/>
    </div>
    }
    is_start_page={true}/>
  );
